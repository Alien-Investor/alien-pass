...
const APP_VERSION = '1.12';   // Anzeige in den Einstellungen; muss VERSION_NAME entsprechen (build-www.sh setzt es aus VERSION, roundtrip-test.mjs prüft es)
...
const T = {
  ...
  "bio.rearmNeeded":{de:"Fingerabdruck nach dem Update bitte einmal in den Einstellungen neu aktivieren — ab jetzt erkennt die App auch über einen Neustart hinweg, ob ein fremder Finger registriert wurde.",en:"After the update, please re-enable fingerprint unlock once in Settings — from now on the app notices a newly enrolled finger even across a reboot."},
  ...
  "what.user":{de:"Nutzername",en:"Username"},"what.email":{de:"E-Mail",en:"E-mail"},"what.pass":{de:"Passwort",en:"Password"},"what.sel":{de:"Markierung",en:"Selection"},"what.url":{de:"URL",en:"URL"},
...
const App = (function(){
  ...
  // kontrolliert ein Angreifer die Uhr. Vorher galt der Eintrag nur während einer laufenden Wartezeit, und jeder App-Neustart danach begann
  // wieder bei 0 — die Wartezeit wuchs nie über 2 s (Querfund Sachwert-Tresor Audit run-5 #2).
  ...
  // Übernehmen: Zähler als Maximum aus RAM und Speicher (auch nach abgelaufener Wartezeit; zweiter Tab), Wartezeit höchstens 30 s voraus —
  // ein Eintrag weit in der Zukunft (Uhr zurückgestellt, Müll) wird gekappt statt verworfen. Unplausibles → ignoriert.
  ...
  // Nachgeholter Import (v1.10, Gerätetest): der Datei-Picker ist eine fremde Android-Activity — bei „Sperren im Hintergrund: sofort“ sperrt die
  // App beim Öffnen des Pickers, die Datei kam bisher in eine gesperrte App und wurde STILL verworfen (Import unmöglich). Jetzt: nur den
  // Dateiverweis merken (Handle, kein Inhalt; nichts wird gelesen, solange die App zu ist), Toast auf dem Sperrbildschirm, nach dem Entsperren
  // (enterApp) im Sicherung-Tab denselben Handler mit derselben Datei aufrufen. Verfällt nach PENDING_FILE_MS ohne Entsperren. Die Sperr-
  // Regel selbst bleibt unangetastet — bewusst KEINE Schonfrist mit offenem Schlüssel im Hintergrund (Entscheidung Nutzer 26.09.2026).
  ...
  function runPendingFile(){ const p=pendingFile; pendingFile=null; if(!p||!VAULT||Date.now()-p.at>PENDING_FILE_MS) return;
    const fn=p.id==='csv-file'?importCsv:p.id==='proton-file'?importProton:p.id==='vault-file'?importVault:null; if(!fn) return;
  ...
  function importCsv(ev){
    const f=ev&&ev.target&&ev.target.files&&ev.target.files[0]; if(!f) return; if(!VAULT){ deferFile(ev); return; } const input=ev.target; $('csv-msg').textContent='';
  ...
  function importProton(ev){
    const f=ev&&ev.target&&ev.target.files&&ev.target.files[0]; if(!f) return; if(!VAULT){ deferFile(ev); return; } const input=ev.target; $('proton-msg').textContent=''; cancelProton();
  ...
  // bioGen ist seit v1.8 die Generation ALLER Pforten (Passphrase, Fingerabdruck, PIN): jede Sperre und jeder Passphrase-Wechsel
  // erhöht sie, jede laufende Pforte pinnt sie vor dem ersten await und verwirft ihr Ergebnis, wenn sie sich geändert hat.
  // Nicht umbenannt, weil die Invarianten (BIO-INVARIANTEN.md, Audit run-3) sie so nennen.
  // bioKeep (v1.8) ist NUR Anzeige: die Wahrheit steht im Slot des Plugins und hängt in dessen GCM-AAD (boot+"|keep"). Geändert wird sie
  ...
  // Warnung „neuer Fingerabdruck im System“ (Querfund Tresor-Audit run-5 #1): bleibt stehen, bis der Nutzer sie liest oder bewusst neu
  // aktiviert — ein Toast ging im Tresor-Gerätetest beim Aufbau der Ansicht unter. Nur ein Marker, nichts Geheimes.
  const BIO_ALERT_KEY='ai-pass-bio-alert';
  ...
  async function bioProbe(auto){
    ...
    bioKeep=!!(st&&st.enabled&&st.keep);   // nur Anzeige (v1.8): gilt der Slot über einen Neustart hinaus?
    // Neue Registrierung im System schlägt Neustart (Querfund Tresor-Audit run-5 #1): das Plugin prüft den Kanarien-Schlüssel VOR der
    // Boot-Kennung. Kein automatisches Neu-Bewaffnen — der Nutzer aktiviert bewusst neu (und sieht, dass sich etwas geändert hat).
    ...
    if(reason==='reboot'||(!blob&&marker&&reason==='none')){                             // Neustart: nativ ist der Slot schon weg; JS-Blob verwerfen, Marker + Kanarie bleiben
      if(blob||reason==='reboot') bioDrop(false); setBioMarker(true); bioNeedsRearm=true; bioMsg(tr('bio.afterReboot')); return; }   // disable() würde auch die Kanarie löschen (run-5 #1)
  ...
  async function bioArm(dekX, kdf, wrap, rearm, keep){
    ...
    try{
      ...
      // versuchen — Kanarie und Marker bleiben, sonst ginge der Nachweis einer neuen Registrierung verloren
      if(rearm&&c==='unavailable'){ bioArmed=false; setBioMarker(true); bioNeedsRearm=true; toast(tr('bio.naNow')); return false; }
      if(c==='nocanary'){ bioDrop(false); toast(tr('bio.rearmNeeded')); return false; }   // Slot von vor v1.6.1 (ohne Kanarie): bewusst neu aktivieren, kein Fehlalarm
  ...
  /* ---------- Schnell-Entsperren per PIN (v1.8, bis zum Beenden der App) ----------
     ...
     sondern nur eine Kopie des Datenschlüssels, die beim Sperren im Arbeitsspeicher bleibt — unter einem PIN-abgeleiteten
     Argon2id-Schlüssel verpackt (AAD-Rolle 'pin', an m/t/p/Salz der DATEI gebunden wie der Fingerabdruck-Slot). Der Slot lebt
  ...
  async function doPin(){
    ...
    try{
      ...
      if(gen!==bioGen||DEK||pendingUnlock){ $('lock-pin').value=''; maskInputs('#screen-lock'); return; }   // verspäteter Fehlversuch stört keine offene Sitzung
      $('lock-pin').value=''; maskInputs('#screen-lock');
      if(PIN!==slot) return;                                           // Slot ist zwischendurch schon verworfen worden
      PIN.tries++;
  ...
  async function pinEnable(){
    ...
    try{
      ...
      const pkdf={m:KDF.m, t:KDF.t, p:KDF.p, salt:rand(16)};           // Tresor-Parameter (der Benchmark hat sie für dieses Gerät gewählt), frisches Salz
  ...
  // Bewusst gesperrt: Riegel für BEIDE Schnell-Pforten — nächster Start nur mit Passphrase, danach gelten sie wieder.
  // Verwerfen statt Riegeln würde die Strg+L-Gewohnheit bestrafen (jedes Mal neu einrichten).
  function lockNow(){ if(BIO&&(bioArmed||bioBlob())) setBioHold(true); if(PIN) pinHold=true; bioAuto=false; lock(); }
  ...
  function renderSettings(){ if(!VAULT) return; const s=VAULT.settings; $('set-autolock').value=String(s.autolock); $('set-bglock').value=String(s.bgLock); $('set-clip').value=String(s.clipClear); syncCombos();
    ...
    const bc=$('bio-card'); if(bc){ bc.classList.toggle('hidden',!BIO); $('bio-off').classList.toggle('hidden',bioArmed); $('bio-on').classList.toggle('hidden',!bioArmed);
      // zwei fertige Texte statt eines zusammengesetzten: beide tragen data-i18n, applyI18n übersetzt sie, hier wird nur umgeschaltet (v1.8)
    ...
    // PIN-Karte: v1.8 nur am Desktop (Klasse .only-desk im HTML, hier zusätzlich per JS — Android später freischalten, der Code ist plattformneutral)
  ...
  async function changePass(){
    ...
    try{
      ...
      const hadBio=bioArmed||!!bioBlob()||bioMarker(); if(hadBio) bioDrop(true); pinDrop(); bioGen++;   // neuer DEK/Salt: Fingerabdruck-Slot passt nicht mehr → bewusst neu aktivieren, PIN-Slot im RAM ebenso weg; bioGen++ lässt auch einen laufenden enroll verfallen (Audit run-3 #5)
  ...
  return {bioAlertOk,boot,doSetup,doUnlock,doTotp,cancelTotp,lock,tab,newEntry,saveEntry,cancelEdit,editCurrent,deleteCurrent,toggleFavCurrent,openDetail,closeDetail,toggleReveal,copyField,renderList,
