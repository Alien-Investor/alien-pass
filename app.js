...
const App = (function(){
  ...
  async function doSetup(){
    ...
    pendingFile=null;   // ein gemerkter Verweis gehört nie in einen NEU angelegten Tresor (Audit run-9 #4)
  ...
  function runPendingFile(){ const p=pendingFile; pendingFile=null; if(!p||!VAULT) return; const d=Date.now()-p.at; if(d<0||d>PENDING_FILE_MS) return;   // zurückgestellte Uhr verlängert nicht (Audit run-9 #3)
  ...
  function importVault(ev){
    ...
    r.onload=()=>{ input.value=''; if(!VAULT||VAULT!==session) return;   // während des Lesens gesperrt/neu entsperrt → nichts in die gesperrte oder nächste Sitzung tragen (Audit run-9 #2/#5)
  ...
  function importCsv(ev){
    const f=ev&&ev.target&&ev.target.files&&ev.target.files[0]; if(!f) return; if(!VAULT){ deferFile(ev); return; } const input=ev.target, session=VAULT; $('csv-msg').textContent='';
    ...
    r.onload=()=>{ input.value=''; if(!VAULT||!DEK||VAULT!==session) return; const text=String(r.result);   // währenddessen gesperrt oder neue Sitzung → abbrechen (Audit run-9 #5)
  ...
  function importProton(ev){
    const f=ev&&ev.target&&ev.target.files&&ev.target.files[0]; if(!f) return; if(!VAULT){ deferFile(ev); return; } const input=ev.target, session=VAULT; $('proton-msg').textContent=''; cancelProton();
